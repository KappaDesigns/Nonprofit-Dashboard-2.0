...
export class Script implements Resource<string> {
	...
	private getResourceNameFromPath(): string {
		...
		if (nameParts.length == 1) {
			throw new IllegalResourceFile(`Illegal resource file of type "directory" is not type of "js"`);
		} else if (nameParts.length >= 2 && nameParts[nameParts.length - 1] != 'js') {
			throw new IllegalResourceFile(`Illegal resource file of type "${nameParts[1]}" when it should be "js"`);
		} else {
			let name: string = "";
